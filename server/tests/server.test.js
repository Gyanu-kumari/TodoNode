...
describe('DELETE /todos/:id', () => {
  it('should remove a todo', done => {
    ...
    request(app)
      ...
      .expect(200)
      .expect(res => {
        exp(res.body.todo._id).toBe(hexId);
      })
      .end((err, res) => {
        if (err) {
          return done(err);
  ...
  it('should update the todo', done => {
    ...
    request(app)
      ...
      .expect(res => {
        ...
        exp(typeof res.body.todo.completedAt).toBe('number');
      })
      .end(done);
  });
  ...
  it('should clear the completedAt when todo is not completed', done => {
    ...
    request(app)
      ...
      .expect(res => {
        ...
        exp(res.body.todo.text).toBe(text);
        exp(res.body.todo.completedAt).toBeFalsy();
      })
      .end(done);
  });
});
...
describe('GET /users', () => {
  it('should create a user', done => {
    ...
    request(app)
      ...
      .expect(200)
      .expect(res => {
        exp(res.headers['x-auth']).toBeTruthy();
        exp(res.body.email).toBe(email);
        exp(res.body._id).toBeTruthy();
      })
      .end(err => {
        if (err) {
          return done(err);
        ...
        User.findOne({ email })
          .then(user => {
            exp(user).toBeTruthy();
            exp(user.password).not.toBe(password);
            done();
          })
          .catch(e => done(e));
      });
...
describe('POST /users/login', () => {
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        User.findById(users[1]._id)
          .then(user => {
            exp(user.toObject().tokens[1]).toMatchObject({
              access: 'auth',
  ...
  it('should not login credentials not match', done => {
    request(app)
      ...
      .expect(400)
      .expect(res => {
        exp(res.headers['x-auth']).toBeFalsy();
      })
      .end(err => {
        if (err) return done(err);
